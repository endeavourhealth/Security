...
public class Realms extends KeycloakAdminClientBase {
    ...
    public List<RealmRepresentation> getRealms() {
        ...
        try (CloseableHttpClient httpClient = HttpClientBuilder.create().build()) {
            HttpResponse response = doGet(httpClient, getAuthServerBaseUrl() + "/admin/realms");
    ...
    public RealmRepresentation getRealm(String realmId) {
        ...
        try (CloseableHttpClient httpClient = HttpClientBuilder.create().build()) {
            HttpResponse response = doGet(httpClient, getAuthServerBaseUrl() + "/admin/realms/" + realmId);
    ...
    public RealmRepresentation postRealm(RealmRepresentation role) {
        ...
        try (CloseableHttpClient httpClient = HttpClientBuilder.create().build()) {
            HttpResponse response = doPost(httpClient, getAuthServerBaseUrl() + "/admin/realms", role);
            role = toEntity(response, realmRepresentationTypeReference);
        } catch (IOException e) {
    ...
    public void deleteRealm(String realmId) {
        ...
        try (CloseableHttpClient httpClient = HttpClientBuilder.create().build()) {
            HttpResponse response = doDelete(httpClient, getAuthServerBaseUrl() + "/admin/realms/" + realmId);
        } catch (IOException e) {
